...
SPEC-0019 adds `record-task-response`, which accepts only the deterministic `.sundial/<UserAnnotationId>response.md` handoff path announced in the active assignment; exposing that derived path is its narrow exception to this slice's identity-hiding rule. SPEC-0020 adds `annotate` and retains implicit-assignment resolution without agent-supplied identities. The agent-facing CLI never exposes list, enqueue, ready, claim, complete, requeue, rename, session ensure, transcript, open, interrupt, reset, companion reads/deletes, provider selection, or arbitrary work/session lookup.
...
- 2026-07-20: Made prompt completion target-aware: selecting `%Q` now offers current CLI-owned agent slots with names and scope, typed slot/name prefixes remain selectable, and accepting a target preserves it before opening the composer. Applied the editor 0.7.1 patch increment; no new durable Decision Record was needed.
- 2026-07-20: Removed provider reconciliation from `agent list`, so prompt targeting, Send readiness, and queue dispatch use only CLI-persisted agent/session state. Successful Codex capability probes now persist in machine-local configuration for 24 hours, keyed by CLI version, executable path, and Codex version, with `health --refresh` as an explicit bypass. Applied the CLI 0.3.1 patch increment and proposed CAND-0001/CAND-0002 for the durable queue-readiness and capability-cache boundaries.
...
- 2026-07-20: Target-aware prompt completion passed `npm run check-types`, `npm run lint`, 41 CLI unit tests, 74 editor unit tests, 10 Codex app-server integration tests, and all 3 pinned VS Code 1.118.1 scenarios. The staged prompt scenario verifies `%F` offers Bob/Amy slot choices and `%F>1` opens the composer targeted to Bob.
...
- 2026-07-20: Persisted-only queue readiness and the machine-local capability cache passed the broad regression set: `npm run check-types`, `npm run lint`, 42 CLI unit tests, 74 editor unit tests, 11 Codex app-server integration tests, and all 3 pinned VS Code 1.118.1 scenarios. Coverage verifies that `agent list` performs no provider session read, enqueue requires only a persisted available session, and separate adapter processes reuse the probe until forced refresh, 24-hour expiry, or a Codex-version change.
