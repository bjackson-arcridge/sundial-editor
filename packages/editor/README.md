...
The annotation toolbar can also open a response composer for either a user or agent annotation. Sundial preserves the original task type and source scope while creating the response as a new queued interaction. When the originating agent conversation is still active it is preselected; otherwise the composer requires you to choose an agent and explains that the selected agent may not have the prior transcript.
