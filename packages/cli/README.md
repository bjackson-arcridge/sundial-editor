...
The editor executable accepts structured JSON for its machine commands and exposes health, annotation, agent, work-queue, transcript, open, interrupt, reset, and Git workflow operations. The workflow surface reads and moves a first-parent diff baseline, creates current-file or all-file temporary checkpoints, consolidates the temporary suffix and dirty work into a permanent commit, and repairs annotation companions for Git-reported source moves and deletes. Git is invoked with structured arguments, and mutations reject unresolved conflicts, in-progress operations, unsafe temporary stacks, and any temporary commit reachable from `origin`.
...
Annotation commands store linked user and agent source annotations in compact version-4 companions under the workspace's mirrored `.sundial/` tree. Each annotation records its immutable permanent-base commit. Reads return the CLI-resolved current permanent commit and exact annotation-ID membership used by the editor filter. Valid version-3 companions migrate to version 4 on their next validated write without changing annotation, official-response, or cross-file-link identities; malformed companions remain untouched.
